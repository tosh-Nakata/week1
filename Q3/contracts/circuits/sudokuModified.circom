...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            range_puzzle[i][j] = RangeProof(200);
            range_solution[i][j] = RangeProof(200);
        }
    }
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            // component range_puzzle[i][j] = RangeProof(200);
            // component range_solution[i][j] = RangeProof(200);
            range_puzzle[i][j].range[0] <== 0;
            range_puzzle[i][j].range[1] <== 9;
            range_puzzle[i][j].in <== puzzle[i][j];
            range_puzzle[i][j].out === 1; //[assignment] change assert() to use your created RangeProof instead
